...
public class JdbcPriceRecordRepository implements PriceRecordRepository {
    ...
    public PriceRecord save(PriceRecord record) {
        ...
        jdbcTemplate.update(connection -> {
            PreparedStatement ps = connection.prepareStatement(
                    "INSERT INTO price_records (segment_id, price, currency, recorded_date, purchased) VALUES (?, ?, ?, ?, FALSE)",
            ...
            ps.setDate(4, Date.valueOf(record.getRecordedDate()));
            return ps;
        }, keyHolder);

        Long id = keyHolder.getKey().longValue();
        return new PriceRecord(id, record.getSegmentId(), record.getPrice(), record.getRecordedDate(), false);
    }

    @Override
    public List<PriceRecord> findBySegmentId(Long segmentId) {
        return jdbcTemplate.query(
                "SELECT id, segment_id, price, currency, recorded_date, purchased FROM price_records " +
                "WHERE segment_id = ? ORDER BY recorded_date DESC, id DESC",
    ...
    public Optional<PriceRecord> findLatestBySegmentId(Long segmentId) {
        List<PriceRecord> results = jdbcTemplate.query(
                "SELECT id, segment_id, price, currency, recorded_date, purchased FROM price_records " +
                "WHERE segment_id = ? ORDER BY recorded_date DESC, id DESC LIMIT 1",
                ROW_MAPPER, segmentId
        );
        return results.stream().findFirst();
    }
    ...
    public Optional<PriceRecord> findById(Long id) {
        List<PriceRecord> results = jdbcTemplate.query(
                "SELECT id, segment_id, price, currency, recorded_date, purchased FROM price_records WHERE id = ?",
                ROW_MAPPER, id
        );
        return results.stream().findFirst();
    }
    ...
    public PriceRecord update(PriceRecord record) {
        jdbcTemplate.update(
                "UPDATE price_records SET price = ?, currency = ?, recorded_date = ?, purchased = ? WHERE id = ?",
                record.getPrice().amount(),
                record.getPrice().currency().name(),
                Date.valueOf(record.getRecordedDate()),
                record.isPurchased(),
                record.getId()
        );
        return record;
